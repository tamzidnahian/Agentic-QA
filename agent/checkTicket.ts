...
if (!ticketKey) {
  console.error("Usage: npm run agent:check-ticket -- SCRUM-1");
  process.exit(1);
}
...
async function main() {
  ...
  const warnings = [
    ...
    {
      ok: /acceptance criteria/i.test(description),
      text: 'Description should include an "Acceptance criteria" section.',
    },
    {
      ok: /\b(open|go to|navigate|visit)\b/i.test(description),
      text: "Description should say which page or route to open.",
    },
    {
      ok: /\bvisible|shown|displayed|contains|url\b/i.test(description),
      text: "Description should include visible UI expectations or URL checks.",
    },
    {
